...
if ($urunsor->rowCount()==0) {
	...
	header("Location:index.php?durum=gecersizürün");
	exit;
}
?>
...
<?php } 
elseif ($_GET['durum']=="yetersizStok") {?>
		...
		<div class="col-md-9"><!--Main content-->
			...
			<div class="row">
				<div class="col-md-6">
					...
					<?php
					$urun_id=$uruncek['urun_id'];
					$urunfotosor=$db->prepare("SELECT * FROM urunfoto where urun_id=:urun_id order by urunfoto_sira ASC limit 1,3 ");        // ALTTA BULUNAN 3 ADET RESİM !
					$urunfotosor->execute(array(
						'urun_id' => $urun_id
						));
				...
				<div class="col-md-6 det-desc">
					<div class="productdata">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<input type="number" min="1" max="5" class="form-control" value="1" name="urun_adet">
							</div>
							<?php if(!empty($kullanicicek['kullanici_id'])) {?>
							<input type="hidden" name="kullanici_id" value="<?php echo $kullanicicek['kullanici_id'] ?>"> <?php } ?>
							...
							<div class="col-sm-4">
								...
								<button type="submit" name="sepetekle" class="btn btn-default btn-red btn-sm"><span class="addchart">Sepete Ekle</span></button> <?php }
								else{ ?>
								<button type="submit" name="sepetekle" disabled="" class="btn btn-default btn-red btn-sm"><span class="addchart">Ürün Kalmadı</span></button> 
								<?php } }
								else if(!isset($_SESSION['userkullanici_mail'])){?>
								<button type="submit" name="sepetekle" disabled="" class="btn btn-default btn-red btn-sm"><span class="addchart">Giriş Yapın</span></button> <?php }?>
			...
			<div class="tab-review">
				<ul id="myTab" class="nav nav-tabs shop-tab">
					...
					<?php  if(empty($_GET['durum'])) { 
						...
						<?php 
						if(!empty($kullanicicek['kullanici_id'])){
						...
						<div id="myTabContent" class="tab-content shop-tab-ct">
							...
							<div class="tab-pane fade <?php if(!empty($_GET['durum'])) { if ($_GET['durum']=="ok") {?>
								active in
								<?php }} ?>" id="rev">
